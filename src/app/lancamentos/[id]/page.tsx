...
export default function Dashboard() {
  ...
  // AQUI ESTÁ A MUDANÇA: Substituímos 'any' pelo tipo 'FormData'
  const handleSalvar = async (dadosDoFormulario: FormData, arquivo: File | null) => {
    ...
    Object.keys(dadosDoFormulario).forEach(key => {
      formData.append(key, String(dadosDoFormulario[key])); // Converte tudo para string para o FormData
    });
    if (arquivo) {
    ...
    } catch (error) { // AQUI TAMBÉM CORRIGIMOS O 'catch'
  ...
  const handleAbrirDialogParaEditar = (lancamento: Lancamento) => {
    // Precisamos garantir que os tipos são compatíveis (FormData aceita string | number)
    const dadosParaEditar: FormData = { ...lancamento };
    setLancamentoParaEditar(dadosParaEditar);
    setIsDialogOpen(true);
  };
