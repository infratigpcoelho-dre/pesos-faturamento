// Arquivo: src/app/lancamentos/[id]/page.tsx (NOVA PÁGINA DE DETALHES)
...
export default function LancamentoDetalhePage() {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Lançamento não encontrado'); // O backend deu um erro
    const data = await response.json();
    setLancamento(data);
  } catch (error) {
    toast.error("Erro ao carregar detalhes do lançamento."); // <-- FOI ISSO QUE ACONTECEU!
    router.push('/'); // E então ele te jogou de volta para o dashboard
  }
    }
    carregarDetalhes();
  }, [id, router]);
