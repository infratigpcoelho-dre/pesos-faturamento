// Arquivo: src/app/login/page.tsx (SALVANDO A PLACA)
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // MUDANÇA: Salvamos a placa também
      localStorage.setItem('authToken', data.token);
      localStorage.setItem('userRole', data.role); 
      localStorage.setItem('userFullName', data.nome_completo || '');
      localStorage.setItem('userPlacaCavalo', data.placa_cavalo || ''); // <--- NOVO
    ...
    <div className="flex items-center justify-center min-h-screen bg-background">
      <Card className="w-full max-w-sm">
        <CardHeader className="items-center text-center">
          ...
          <CardDescription>Entre com seu usuário e senha para acessar.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          ...
          <div className="mt-4 text-center text-sm">
            ...
            <a href="https://wa.me/55996908384" target="_blank" rel="noopener noreferrer" className="underline hover:text-primary">Fale com o suporte</a>
          </div>
        </CardContent>
      </Card>
    </div>
